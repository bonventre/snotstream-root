...
(function($) {
    ...
    Axis.prototype.quickdraw = function(ctx, plot) {
        // Memory Leaks patch
        if (this.__ticks) {
          this.__ticks = null;
        }

        return this.renderer.quickdraw.call(this, ctx, plot);
    ...
    Axis.prototype.quickpack = function() {
        if (this.show) {
            this.renderer.quickpack.call(this);
        }
        // these properties should all be available now.
        if (this._min == null) {
            this._min = this.min;
    ...
    function jqPlot() {
        ...
        this.replot = function(options) {
            ...
            if (resetAxes.length){
              for (var i=0;i<resetAxes.length;i++){
                name = resetAxes[i];
                // Memory Leaks patch : clear ticks elements
                var t = this.axes[name]._ticks;
                for (var i = 0; i < t.length; i++) {
      ...
      // clear - false to not clear (empty) the plot container before redrawing (default: true).
        ...
        this.quickdraw = function(resetAxes){
          resetAxes = (resetAxes != null) ? resetAxes : [];
                                      ...
                                      //if (this._gridPadding[arr[n]] == null && gridPadding[arr[n]] > 0) {
                                        ...
                                        //else if (this._gridPadding[arr[n]] == null) {
                                          ...
                                          if (resetAxes.length){
                                            var ax = this.axes;
                                            for (var i=0;i<resetAxes.length;i++){
                                              ax[resetAxes[i]].quickdraw(this.baseCanvas._ctx,this);
                                              ax[resetAxes[i]].set();
                                            }

                                            for (var i=0;i<resetAxes.length;i++){
                                              ax[resetAxes[i]].quickpack();
                                            }
                                          }
...
return rgb;
    ...
    $.jqplot.LinearAxisRenderer.prototype.quickdraw = function(ctx, plot) {
      if (this.show) {
        // populate the axis label and value properties.
        // createTicks is a method on the renderer, but
        // call it within the scope of the axis.
    ...
    $.jqplot.LinearAxisRenderer.prototype.quickpack = function() {
      // Add defaults for repacking from resetTickValues function.
      var ticks = this._ticks;
      var max = this.max;
      var min = this.min;
      var offmax = this._offsets.max;
      var offmin = this._offsets.min;
